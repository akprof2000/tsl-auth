...
| `petrov` | согласующий |
| `sidorova` | руководитель: утверждает |
...
1. Войти как `ivanova`, создать документ и отправить на согласование; затем войти как `petrov` и `sidorova` — согласовать и утвердить.
2. Войти как `admin-doc` → «Пользователи»: создать сотрудника, выдать роль, одобрить заявку.
3. Выйти и нажать «Зарегистрироваться»: новый сотрудник запросит роль, после одобрения — «Обновить права».
