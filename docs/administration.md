...
| `security-bot` | для бота мессенджера: то же плюс блокировка учётной записи и принудительная смена пароля (`user_lock`, `password_force`) |
| `security-officer` | пользователь, который через бота может заблокировать чужую учётную запись и потребовать смену пароля |
...
Email у каждого пользователя свой: второй учётной записи с тем же email не создать (ни администратору, ни при
самостоятельной регистрации). Логин с `@` допускается, только если он совпадает с email этой же учётной записи.
...
| **Одноразовый пароль** | генерируется и показывается один раз; при первом входе пользователь обязан его сменить — до этого ему недоступны админка и личный кабинет |
...
Число ожидающих заявок показывается в меню. На одну роль у пользователя может быть только одна ожидающая заявка
(повторная отправка формы не создаёт дубликат; дубли из старых версий при обновлении закрываются как отклонённые
с пометкой `system:dedupe`).
...
Отклонённые запросы к Admin/App API пишутся со своим кодом (400/404/409, уровень info); `security.alert` порождают
только ошибки сервера (5xx), а не ошибки ввода.
...
Политика паролей, сроки жизни токенов и максимальный срок сессии (после него refresh не продлевает вход — нужен
повторный вход), персональные токены, сброс через бота, сроки хранения данных (в т.ч. по каждому типу событий журнала),
ручной запуск очистки. Подробно — [конфигурация](configuration.md#настройки-в-бд).

Пока политика токенов не сохранена ни разу, сроки берутся из параметров запуска `Auth__AccessTokenLifetimeMinutes`,
`Auth__RefreshTokenLifetimeDays` и др.; после первого сохранения действуют только значения со страницы «Настройки».

## Личный кабинет пользователя

Доступен любому вошедшему пользователю (ссылки внизу страниц входа), кроме пользователя с временным паролем — ему
сначала нужно задать постоянный (`/Account/ChangePassword`):
