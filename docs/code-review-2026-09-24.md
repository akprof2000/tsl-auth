...
## Статус исправлений (ветка `fix/code-review`)

Исправлены все пункты, кроме отмеченных ниже как осознанно оставленные или сделанные частично. Каждое исправление
кода покрыто тестом: unit 86/86, интеграционные 118/118 (SQLite и PostgreSQL), в том числе отдельные наборы
`ReviewRegressionScenarios`, `ReviewCoverageScenarios`, `AuthorizationCodeScenarios` и `ReviewFixesTests`.

| Пункт | Решение |
|---|---|
| L8 | Оставлено осознанно: временный пароль первого администратора в логе — штатный способ первого входа в закрытом контуре; пароль одноразовый (смена при первом входе), а `Bootstrap__AdminPasswordFile` позволяет обойтись без лога вовсе. |
| L40 | Частично: образы инструментов CI (Dockle, mermaid-cli, k6) — по digest; `postgres` и `nginx` в compose — по тегу: после `docker load` в закрытом контуре ссылка по digest находится не во всех версиях Docker. `export-images.ps1` записывает digest перенесённых образов, документация уточнена. |
| L42 | `cleanup.yml` — поведение, заданное владельцем репозитория, не менялось; в `ci.yml` добавлен комментарий о холодном кэше сборки. |
| L43 | `prune-releases.yml` нормализован в LF (содержимое не изменилось); сам процесс выпуска релизов оставлен владельцу. |
| L35 | Одноразовые секреты в TempData оставлены: cookie зашифрована Data Protection и живёт один редирект; показ без редиректа при обновлении страницы создавал бы приложение повторно. |
| M6 | Частные сети для вебхуков по умолчанию **разрешены** (получатели в закрытом контуре находятся именно там); всегда запрещены loopback, link-local, multicast, `0.0.0.0`; `Webhooks__AllowedNetworks` сужает список. |
