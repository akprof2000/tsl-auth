...
выполнить запрос — `/docs/api`. Рабочие примеры на пяти стеках — в [`samples/`](../samples).
...
* Выход: `GET /connect/logout?id_token_hint=...&post_logout_redirect_uri=...`.
...
и попадает в ленту событий как `security.alert`. Демо бота со встроенным чатом — [`samples/docflow-demo`](../samples/docflow-demo/).
