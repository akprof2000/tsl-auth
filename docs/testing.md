...
Unit- и интеграционные тесты выполняются в CI на каждый push и pull request. Полный стенд (UI, нагрузка, отказы) —
workflow `E2E`: вручную, еженедельно и обязательно перед публикацией образа на push в `main` и релизные теги —
без него `latest` не обновляется (≈ 8 мин). Журналы демо-приложений и контейнеров стенда сохраняются в артефакт
...
### Обновление версии на живых данных

`tests/TslAuth.IntegrationTests/UpgradeScenarios.cs`, на SQLite и PostgreSQL. Данные создаются обычными сценариями
(приложения, матрица прав, пользователи с паролями, сервисный клиент, вход с выдачей токенов), затем сервис
останавливается, а схема откатывается штатными Down-миграциями до **первой версии** (`InitialCreate`): новые столбцы
и таблицы удаляются, данные в таблицах первой версии остаются. После этого запускается текущая версия.

| Проверка после обновления | Результат |
|---|---|
| Схема докатана автоматически, все миграции применены | ✅ |
| Ключи подписи те же (JWKS не изменился) | ✅ |
| access-токен, выданный **до** обновления, принимается (`/connect/userinfo`) | ✅ |
| refresh-токен, выданный **до** обновления, работает, права те же | ✅ |
| Вход со старым паролем; роли и разрешения из матрицы на месте | ✅ |
| Сервисный клиент — старый секрет и старые права | ✅ |
| Зашифрованные логин и email читаются, смена пароля не требуется (даже при включённом сроке действия пароля) | ✅ |
| Новые функции работают на старых данных (история паролей) | ✅ |
| Повторный старт на обновлённой БД ничего не меняет | ✅ |
