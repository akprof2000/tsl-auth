...
| Саморегистрация | Кнопка на стартовой странице, заявка на роль, экран ожидания, «Обновить права» после одобрения |
...
1. На стартовой странице демо нажмите «Зарегистрироваться» — откроется форма TSL Auth для приложения
   «Документооборот» (саморегистрацию и запрашиваемые роли включает `seed.ps1`).
2. Новый сотрудник задаёт логин, пароль и выбирает роль: «Сотрудник» или «Согласующий».
   После регистрации он сразу входит в демо, но без прав — видит экран «Заявка на рассмотрении».
3. Администратор документооборота (`admin-doc`) одобряет заявку: «Пользователи» → «Заявки».
4. Сотрудник нажимает «Обновить права» — получает новый токен с ролью, пароль повторно не спрашивается.

Какие роли можно запросить, настраивается в TSL Auth: флаг «запрашиваемая» у роли в матрице `docflow-api`.
